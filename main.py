...
def run_day_trip_generator():
    ...
    current_trip = generate_random_trip(options_list)
    print_full_trip(current_trip)
    finalize_trip(current_trip,options_list)

def finalize_trip(current_trip,options_list):
    complete = ""
    while complete !="Y":
...
def change_option(current_trip, options_list):
    ...
    if change == "Destination":
        ...
        current_trip[1] = random.choice(options_list[1])
    elif change == "Transportation":
        current_trip[2] = random.choice(options_list[2])   
    elif change == "Entertainment":
        current_trip[3] = random.choice(options_list[3])
    return current_trip
...
def print_full_trip(trip_list):
    ...
    print(f"Transportation: {trip_list[2]}")
